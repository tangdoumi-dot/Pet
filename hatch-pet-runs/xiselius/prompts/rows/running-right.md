Create one horizontal animation strip for Codex pet `xiselius`, state `running-right`.
...
Identity: same pet in every frame: 同一个西塞留斯形象：Q版二头身像素风少年，蓬松白发，金色眼睛，单侧金色坠饰耳环，深蓝衬衫，深色裤子，棕色披风外套，慵懒傲娇、冷淡温柔、毒舌但可爱。动作幅度轻微、柔软、慢动作。. Preserve silhouette, face, proportions, markings, palette, material, style, and props.
Style: Pet-safe sprite: compact full-body mascot, readable in a 192x208 cell, clear silhouette, simple face, stable palette/materials, and crisp edges for chroma-key extraction. Style `pixel`: Pixel-art-adjacent digital mascot with a chunky silhouette, simple dark outline, limited palette, flat cel shading, and visible stepped edges. User style notes: clean chibi pixel art, transparent desktop pet sprite after chroma key removal, crisp edge, consistent proportions and colors, no 3D, no realism, no complex background.
